...
plnmonitor_container_name=`docker compose ps plnmonitordb | tail -n +2 | awk '{ print $1 }'`;
...
docker compose run plnmondaemon /usr/bin/java -Xmx3G -jar /opt/plnmonitor-daemon.jar config
